...
if (registerForm) {
    registerForm.addEventListener('submit', async (e) => {
        ...
        if (passwordInput && passwordInput.value.length <= 6) {
            showToast('Password must be more than 6 characters long.', 'error');
            return;
        }
...
// Bug Report System
document.addEventListener('DOMContentLoaded', () => {
    // 1. Inject Navbar Button
    const navLinks = document.querySelector('.nav-links');
    if (navLinks) {
        const bugBtn = document.createElement('button');
        bugBtn.className = 'btn btn-secondary';
        bugBtn.style.padding = '0.5rem';
        bugBtn.style.display = 'flex';
        bugBtn.style.alignItems = 'center';
        bugBtn.title = 'Report a Bug';
        bugBtn.innerHTML = '🐞';
        bugBtn.onclick = () => openModal('bug-report-modal');

        const profileBtn = navLinks.querySelector('a[href="profile.php"]');
        if (profileBtn) {
            navLinks.insertBefore(bugBtn, profileBtn);
        } else {
            navLinks.appendChild(bugBtn);
        }
    }
    ...
    <div class="modal-overlay" id="bug-report-modal">
        <div class="modal-content glass">
            <div class="modal-header">
                <h2>Report a Bug</h2>
                <button class="close-btn" onclick="closeModal('bug-report-modal')">&times;</button>
            </div>
            <form id="bug-report-form">
                ...
                <div class="form-group">
                    <label>What happened vs. What did you expect?</label>
                    <textarea name="description" rows="3" required></textarea>
                </div>
                <div class="form-group">
                    <label>Steps to reproduce (1, 2, 3...)</label>
                    <textarea name="steps_to_reproduce" rows="3" required></textarea>
                </div>
                <div style="display: flex; gap: 1rem;">
                    <button type="submit" class="btn btn-primary" style="flex: 1;">Submit Report</button>
                    <button type="button" class="btn btn-secondary" style="flex: 1;" onclick="closeModal('bug-report-modal')">Cancel</button>
                </div>
            </form>
        </div>
    </div>
    `;
    document.body.insertAdjacentHTML('beforeend', bugModalHtml);
    ...
    if (bugForm) {
        bugForm.addEventListener('submit', async (e) => {
            ...
            try {
                const res = await fetch(`${API_BASE}/submit_bug.php`, { method: 'POST', body: formData });
                const data = await res.json();
                if (data.status === 'success') {
                    showToast(data.message, 'success');
...
// Authentication Extensions (Forgot Password & OTP)
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (forgotForm) {
        forgotForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const data = await res.json();
                if (data.status === 'success') {
                    const devAlert = document.getElementById('forgot-dev-alert');
                    const link = document.getElementById('forgot-reset-link');
                    // Dynamic absolute URL so it links correctly regardless of base directory
                    const absoluteUrl = window.location.origin + window.location.pathname.replace(/[^\\/]*$/, '') + data.dev_mode_link;
    ...
    if (otpReqForm) {
        otpReqForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const formData = new FormData(otpReqForm);
            try {
